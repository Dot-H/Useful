...
- [Semi-join executor fast paths (GREAT-21)](great21-semijoin-executor-fast-paths.md) -- Phase A "slower semi-join" is expected: zero-binding 1-row ARM gate hits 3 executor gaps (no lazy filter on base, probe-first, row copy); fix = existence gate + empty-base (PR 1) and base lazy filter (PR 2), started 2026-09-10
- [InnerJoinToSemiJoin + pushdown viewdiff regression (GREAT-90)](great90-semijoin-pushdown-viewdiff-regression.md) -- Phase A neutral; Phase B Lists deterministic 54-view blowup, ROOT CAUSE plan-verified: SemiJoinPushdown parks the semi-join between Filter and Scan, ImpResult drops PushableFilters on a semi-join base -> filter loses modality scopes, full-shard download; fix = Filter barrier or re-run FilterPushdown after line 220
