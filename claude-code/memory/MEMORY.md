...
- [Personal lowercase-branch git hook](personal-lowercase-branch-hook.md) — pre-commit script in ~/Useful, symlinked to monorepo .git/hooks/pre-commit, no global core.hooksPath (breaks git-lfs)
...
- [Data-proxy cold replica routing](data-proxy-cold-replica-routing.md) -- slow Dataset Load / ~1s CoalescePartitionsExec with few rows = cold data-proxy pod; join plan FileId to log file_id; 10 MB pages fetched serially (parallelism=1), ~50x read amplification; routing not cache-aware; smaller pages alone (fix 3) is net negative, big serial reads are 40% of wall
- [Data-proxy cache warm-up strategy](data-proxy-cache-warmup-strategy.md) -- only 2 narrow triggers (hot replication <60 min, snapshot RO compaction; leveled compaction warms nothing), all 3+3 ring candidates, interactive pool only, no warm-up at pod start
- [Data-proxy access-plan prefetch sizing](data-proxy-prefetch-access-plan.md) -- cold >=3-miss reads = 0.42% of interactive requests but 21.8% of wall; realistic ImpDownload gain 2.4-4.7% of wall; opener not pluggable, reader factory is; loses 0-match projection skip
