...
- [Data-proxy cold replica routing](data-proxy-cold-replica-routing.md) -- slow Dataset Load / ~1s CoalescePartitionsExec with few rows = cold data-proxy pod; join plan FileId to log file_id; 10 MB pages fetched serially (parallelism=1), ~50x read amplification; routing not cache-aware
