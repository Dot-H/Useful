...
**Follow-up (466 backlog orgs vs 45 dispatched):** the FormulaDiff backlog is a CUSTOM implementation in `FormulaDiffRepository.cs` (over `formula_diff_task`, not the generic `DatasetAbstractJobSubmissionBacklog`). `GetBacklogSizePerOrg` (the "items in backlog" gauge) counts `flags = 0` with NO run-status filter, while `ReserveJobs`/`ReserveJobsForOrganization` require `INNER JOIN formula_diff_job run ... run.job_status = Active` (`FormulaDiffRunStatus`: Initiating=0/Active=1/Paused=2/Done=3). Tasks of non-Active runs are counted but never dispatchable -> ~421 orgs frozen (e.g. f84befa7 pinned at exactly 59074 rows for 3+ days, zero dispatch logs in 48h, zero stale releases, zero errors). Paused has no code setter (manual only); Initiating sticks forever if FormulaDiffWorker.CreateFormulaDiffTasks fails mid-loop (no such error in 7d). CLOSED via SQL on workspace_db (2026-07-17): job_status=1 (Active) -> 44 orgs / 344,548 pending tasks; job_status=2 (Paused) -> 458 orgs / 7,239,776 pending tasks. The 458 starved orgs' runs were manually PAUSED (no code setter exists); paused backlog is 21x the active one. Reactivating runs should scale throughput ~linearly with newly Active orgs (~150-280 diffs/h each) until global IMP capacity (460) binds.
